...
public class GenUtil {
	...
	public static String download(List<ColumnInfo> columns, GenConfig genConfig) throws IOException {
		String tempPath =System.getProperty("java.io.tmpdir") + "query-gen-temp" + File.separator + genConfig.getTableName() + File.separator;
		Map<String,Object> genMap = getGenMap(columns, genConfig);
		TemplateEngine engine = TemplateUtil.createEngine(new TemplateConfig("template", TemplateConfig.ResourceMode.CLASSPATH));
		// 生成后端代码
		List<String> templates = getAdminTemplateNames();
		for (String templateName : templates) {
			Template template = engine.getTemplate("generator/admin/"+templateName+".ftl");
			String filePath = getAdminFilePath(templateName,genConfig,genMap.get("className").toString(),tempPath + "query" + File.separator);
		...
		for (String templateName : templates) {
			Template template = engine.getTemplate("generator/front/"+templateName+".ftl");
			String path = tempPath + "query-web"  + File.separator;
	...
	public static void generatorSql(GenConfig genConfig){
		CaseFormat fromFormat = CaseFormat.LOWER_CAMEL;
		CaseFormat toFormat = CaseFormat.UPPER_CAMEL;
		String menuSql = "INSERT INTO `cc-query`.`sys_menu`(`i_frame`, `name`, `component`, `pid`, `sort`, `icon`, `path`, `cache`, `hidden`, `component_name`, `create_time`, `permission`, `type`) VALUES (b'0', '菜单名称自己写', '%s/index', 36, 32, 'dev', '%s', b'0', b'0', '%s', '%s', NULL, 1);";
		String apiPath = genConfig.getApiPath();
		String[] apiPathArray = apiPath.split("/");
		StringBuffer componentBuffer = new StringBuffer();
		for (String s : apiPathArray) {
			componentBuffer.append(fromFormat.to(toFormat, s));
		}
		log.info("Menu sql:{}",String.format(menuSql,apiPath,apiPath,componentBuffer.toString(), DateUtil.now()));
	}

	public static void generatorCode(List<ColumnInfo> columnInfos, GenConfig genConfig) throws IOException {
		//生成个菜单SQL
		generatorSql(genConfig);
	...
	private static String getFrontFilePath(String templateName, String apiPath, String path, String apiName) {
		...
		if ("api".equals(templateName)) {
			return path  +File.separator +"api" + File.separator +  apiPath + File.separator + apiName + ".js";
		}

		if ("index".equals(templateName)) {
			return path + File.separator +"views" + File.separator + apiPath + File.separator +  "index.vue";
		}
